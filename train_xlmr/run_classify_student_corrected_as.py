...
from processors.snli_custom import SnliProcessor_custom
...
from processors.sst5_custom import SST5Processor_custom
# from processors.gluecos_custom import GluecosProcessor_custom
...
PROCESSORS = {
  ...
  'sst5': SST5Processor_custom,
  'snli': SnliProcessor_custom,
#   'gluecos': GluecosProcessor_custom,
}
...
def compute_metrics(preds, labels):
  ...
  print("Correct predicted label counts are {}".format(label_dict))
  print("False positives are {}".format(fps))
  print("False negatives are {}".format(fns))
  cnt_0 = 0
  cnt_1 = 0
  cnt_2 = 0
  for label in labels:
    ...
    if label == 1: cnt_1 += 1
    if label == 2: cnt_2 += 1
  print("all label counts are: ", cnt_0, cnt_1, cnt_2)
...
def train(args, train_dataset, model, model_teacher, tokenizer, lang2id=None):
  """Train the model."""
  ...
  if args.max_steps > 0:
    ...
    t_total = len(train_dataloader) // args.gradient_accumulation_steps * args.num_train_epochs  # total number of steps
  ...
  #labels_check = []
  for _ in train_iterator:  # will loop number of epoch times
    ...
    for step, batch in enumerate(epoch_iterator):
      ...
      model.train()  # sets the mode to training

      batch = tuple(t.to(args.device) for t in batch)
      # print(batch[len(batch)-1])
      inputs = {"input_ids": batch[0], "attention_mask": batch[1], "labels": batch[3]} #,"label_bool":batch[4]}
      label_bool = batch[4]

      #labels_check.append(inputs["labels"])
      ...
      outputs = model(**inputs)  # passing inputs through the model
      loss = torch.zeros(1, device="cuda", dtype=torch.float32)
      # logger.info("loss init: {}".format(loss.item())) #0.0
      is_labeled = torch.tensor(label_bool, dtype=torch.bool)

      logger.info("Number of labeled examples in the batch:{}".format(inputs["labels"][is_labeled].size(0)))
      logger.info("Number of unlabeled examples in the batch: {}".format(inputs["labels"][~is_labeled].size(0)))
      
      if is_labeled.any().item(): #batch has atleast one labeled instance
        logits_labeled = outputs[1][is_labeled]
        # loss_labeled = inputs["labels"][is_labeled].size(0)*loss_ce(logits_labeled, inputs["labels"][is_labeled])
        loss_labeled = loss_ce(logits_labeled, inputs["labels"][is_labeled])

        loss += loss_labeled
        logger.info("loss_labeled: {}".format(loss_labeled.item()))
      
      if (~is_labeled).any().item(): #batch has atleast one unlabeled instance
        logits_unlabeled = outputs[1][~is_labeled]
        p = F.softmax(torch.div(logits_unlabeled, args.temperature), dim=-1) 
        outputs_teacher = model_teacher(**inputs)#, return_dict=False)
        logits_teacher = outputs_teacher[1][~is_labeled]
        # #compute KL divergence for the unlabeled instances:
        q = F.softmax(logits_teacher, dim=-1) 
        loss_unlabeled = loss_kld(p.log(),q)
        # loss_unlabeled = inputs["labels"][~is_labeled].size(0)*loss_unlabeled
        loss += loss_unlabeled
        logger.info("loss_unlabeled: {}".format(loss_unlabeled.item()))

      loss = (1/(inputs["labels"].size(0)))*loss
      ...
      if (step + 1) % args.gradient_accumulation_steps == 0:
        ...
        #print("labels are {}".format(labels_check))
        #labels_check = []
        optimizer.step()  # update parameters
        scheduler.step()  # Update learning rate schedule
        model.zero_grad()  # to avoid accumulating gradient problem
        global_step += 1
        ...
        if args.local_rank in [-1, 0] and args.save_steps > 0 and global_step % args.save_steps == 0:  # save steps is given as 100
          ...
          if args.save_only_best_checkpoint:  # this is set to true    
            ...
            logger.info(" Dev accuracy {} = {}".format(args.dev_language, result['acc']))
...
def load_and_cache_examples(args, task, tokenizer, split='train', language='en', lang2id=None, evaluate=False):
  ...
  if os.path.exists(cached_features_file) and not args.overwrite_cache:
    ...
    if split == 'train':
      ...
      examples = processor.get_train_examples(args.train_dir,  "train", language)
    elif split == 'swapped':
      ...
      # examples = processor.get_dev_examples("data/sst5", 'mar')  #always use source as eval set
      ...
      # examples = processor.get_test_examples("data/sst5", 'mar')
  ...
  if split == 'train':
    all_label_bool = torch.tensor([f.label_bool for f in features], dtype=torch.long)
  ...
  if args.model_type == 'xlm':
    ...
    if split == 'train':
      dataset = TensorDataset(all_input_ids, all_attention_mask, all_token_type_ids, all_labels, all_label_bool)
    else:
...
def main():
  ...
  parser.add_argument(
    ...
    required=True,
    # help="Path to pre-trained model or shortcut name selected in the list: " + ", ".join(ALL_MODELS),
    help="Path to fine-tuned teacher model or shortcut name selected in the list: ",
  )
  parser.add_argument(
    "--train_language", default="en", type=str, help="Train language if is different of the evaluation language."
  )

  # adding dev language for selecting the best model
  parser.add_argument(
    "--dev_language", default="en", type=str, help="Dev language for selecting the best model."
  )
  parser.add_argument(
  ...
  parser.add_argument("--temperature", default=20, type=float, help="temperature to generate soft targets")
  ...
  if args.do_train:
    ...
    # model = model_class.from_pretrained("/raid/speech/ashish/TSTG_new/results/bnsentiment/bnfinetuning/xlm-roberta-large-LR5e-6-epoch15-MaxLen128/checkpoint-best",num_labels=3)
    ...
    model_teacher = model_class.from_pretrained(args.teacher_model,num_labels=3) 
    ...
    global_step, tr_loss, best_score, best_checkpoint = train(args, train_dataset, model, model_teacher, tokenizer, lang2id)
  ...
  if args.do_train and (args.local_rank == -1 or torch.distributed.get_rank() == 0):
    ...
    # Loading a trained model and vocabulary that you have fine-tuned
  ...
  if args.do_eval and args.local_rank in [-1, 0]: # this is set to true
    ...
    for checkpoint in checkpoints: # both best checkpoint and last checkpoint
      ...
      # result = evaluate(args, model, tokenizer, split='dev', language=args.dev_language, lang2id=lang2id, prefix=prefix, output_file = 'results/marsentiment/mr-zeroshot-expt-2-rand-t1.5-seed12-50k/dev-sst5-mr_preds.50k')
      result = evaluate(args, model, tokenizer, split='dev', language=args.dev_language, lang2id=lang2id, prefix=prefix)
    ...
    output_eval_file = os.path.join(args.output_dir, 'eval_results_sst5-mr.50k')
  ...
  if args.do_predict and args.local_rank in [-1, 0]:
    ...
    model.to(args.device)
    output_predict_file = os.path.join(args.output_dir, args.test_split + '_results_sst5-mr.txt.50k')
    total = total_correct = 0.0
    with open(output_predict_file, 'a') as writer:
      ...
      for language in args.predict_languages.split(','):
        ...
        # result = evaluate(args, model, tokenizer, split=args.test_split, language=language, lang2id=lang2id, prefix='best_checkpoint', label_list=label_list,output_file = 'results/marsentiment/mr-zeroshot-expt-2-rand-t1.5-seed12-50k/test-sst5-mr_preds.50k')
        result = evaluate(args, model, tokenizer, split=args.test_split, language=language, lang2id=lang2id, prefix='best_checkpoint', label_list=label_list)
...
if __name__ == "__main__":
  ...
  logger.info(f"Script took {elapsed_time:.2f} minutes to run")
