...
def train(args, train_dataset_1, train_dataset_2, model, model_teacher, tokenizer, lang2id=None):  #train_dataset(bn) for student
  """Train the model."""
  ...
  train_sampler_1 = SequentialSampler(train_dataset_1) if args.local_rank == -1 else DistributedSampler(train_dataset_1)
  train_dataloader_1 = DataLoader(train_dataset_1, sampler=train_sampler_1, batch_size=args.train_batch_size)

  train_sampler_2 = SequentialSampler(train_dataset_2) if args.local_rank == -1 else DistributedSampler(train_dataset_2)
  train_dataloader_2 = DataLoader(train_dataset_2, sampler=train_sampler_2, batch_size=args.train_batch_size)

  if args.max_steps > 0:
    t_total = args.max_steps
    args.num_train_epochs = args.max_steps // (len(train_dataloader_1) // args.gradient_accumulation_steps) + 1
  else:
    t_total = len(train_dataloader_1) // args.gradient_accumulation_steps * args.num_train_epochs  # total number of steps
  ...
  logger.info("  Num examples = %d", len(train_dataset_1))
  ...
  for _ in train_iterator: # will loop number of epoch times
    epoch_iterator_1 = tqdm(train_dataloader_1, desc="Iteration", disable=args.local_rank not in [-1, 0])
    epoch_iterator_2 = tqdm(train_dataloader_2, desc="Iteration", disable=args.local_rank not in [-1, 0])
    for step, (batch_1,batch_2) in enumerate(zip(epoch_iterator_1, epoch_iterator_2)):
      ...
      if args.model_type != "distilbert":
        inputs_1["token_type_ids"] = (
          batch_1[2] if args.model_type in ["bert"] else None
        )  # XLM don't use segment_ids
      if args.model_type == "xlm":
        inputs_1["langs"] = batch_1[4]

      batch_2 = tuple(t.to(args.device) for t in batch_2)
      inputs_2 = {"input_ids": batch_2[0], "attention_mask": batch_2[1], "labels": batch_2[3]}
      #labels_check.append(inputs["labels"])
      if args.model_type != "distilbert":
        inputs_2["token_type_ids"] = (
          batch_2[2] if args.model_type in ["bert"] else None
        )  # XLM don't use segment_ids
      if args.model_type == "xlm":
        inputs_2["langs"] = batch_2[4]

      outputs = model(**inputs_1)  # passing inputs through the model
      # loss = outputs[0]
      #BF:::
      logits = outputs[1]
      p = F.softmax(torch.div(logits, args.temperature), dim=-1) 
      # print("p:",p)
      outputs = model_teacher(**inputs_2)#, return_dict=False)
      logits = outputs[1]
      q = F.softmax(torch.div(logits, args.temperature), dim=-1) 
      # print("q:",p)
      loss_fct = nn.KLDivLoss()
      loss = loss_fct(p.log(),q)
...
def load_and_cache_examples(args, task, tokenizer, split='train', language='en', lang2id=None, evaluate=False):
  ...
  if os.path.exists(cached_features_file) and not args.overwrite_cache:
    ...
    if split == 'train':
      ...
      examples = processor.get_train_examples(args.data_dir+"/selftraining/2a/", "train", language)
      logger.info("loaded train data from path:{} for language:{}".format(args.data_dir+"selftraining/2a/",language))
    elif split == 'swapped':
...
def main():
  ...
  parser.add_argument("--temperature", default=20, type=int, help="temperature to generate soft targets")
  ...
  if args.do_train:
    ...
    model_teacher = model_class.from_pretrained("/raid/speech/ashish/TSTG_new/results/bnsentiment/enfinetuning/xlm-roberta-large-LR5e-6-epoch15-MaxLen128/checkpoint-best",num_labels=3)
    model_teacher.eval()
    model_teacher.to(args.device)
    train_dataset_1 = load_and_cache_examples(args, args.task_name, tokenizer, split="train", language=args.train_language, lang2id=lang2id, evaluate=False)
    train_dataset_2 = load_and_cache_examples(args, args.task_name, tokenizer, split="train", lang2id=lang2id, evaluate=False) #en
    logger.info("Length of train dataset is {}".format(len(train_dataset_1)))
    global_step, tr_loss, best_score, best_checkpoint = train(args, train_dataset_1, train_dataset_2, model, model_teacher, tokenizer, lang2id)
