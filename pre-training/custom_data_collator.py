...
class DataCollatorForLanguageModeling(DataCollator):
    ...
    def mask_tokens(self, inputs: List[torch.Tensor]) -> Tuple[torch.Tensor, torch.Tensor]:
        ...
        #print("special tokens mask", special_tokens_mask[0])

        probability_matrix.masked_fill_(torch.tensor(special_tokens_mask, dtype=torch.bool), value=0.0)
        probability_matrix.masked_fill_(tomask.eq(False), value=0.0)
        
        #print("probability matrix", probability_matrix[0])
        ...
        #print("bernoulli output", torch.bernoulli(torch.full(labels.shape, 0.8)).bool().shape)
